...
export default function EditAddressModal({ show, onClose, onSave, address }) {
  ...
  // Load provinces and populate form when modal opens or address changes
  useEffect(() => {
    if (show) {
      loadProvinces()
      loadLeafletAssets()
  ...
  useEffect(() => {
    if (address) {
      setFormData(address)
      // Load related PSGC data based on existing address
      if (address.province) {
        loadMunicipalitiesForEdit(address.province, address.municipality)
        if (address.municipality) {
          loadBarangaysForEdit(address.province, address.municipality)
        }
      }
    }
  }, [address])
  ...
  const loadMunicipalitiesForEdit = async (provinceName, selectedMunicipality) => {
    const selectedProvince = provinces.find(p => p.name === provinceName) || 
                            (await psgcService.getAllProvinces()).find(p => p.name === provinceName)
    
    if (selectedProvince) {
      setLoadingMunicipalities(true)
      try {
        const data = await psgcService.getMunicipalities(selectedProvince.code)
        setMunicipalities(data || [])
      } catch (error) {
        console.error('Failed to load municipalities:', error)
      } finally {
  ...
  const loadBarangaysForEdit = async (provinceName, municipalityName) => {
    const allProvinces = provinces.length > 0 ? provinces : await psgcService.getAllProvinces()
    const selectedProvince = allProvinces.find(p => p.name === provinceName)
    
    if (selectedProvince) {
      const municipalityData = await psgcService.getMunicipalities(selectedProvince.code)
      const selectedMunicipality = municipalityData.find(m => m.name === municipalityName)
      
      if (selectedMunicipality) {
        setLoadingBarangays(true)
        try {
          const data = await psgcService.getBarangays(selectedMunicipality.code)
          setBarangays(data || [])
        } catch (error) {
          console.error('Failed to load barangays:', error)
        } finally {
  ...
  const MapPreview = () => {
    const mapId = 'edit-address-map-preview'
      ...
      const timer = setTimeout(() => {
        ...
        const popupContent = `
          <div style="text-align: center; min-width: 180px; padding: 5px;">
            <strong style="color: #2563eb;">${formData.label || 'Address'}</strong><br>
        ...
        <div className="modal-content">
          <div className="modal-header">
            <h5 className="modal-title">
              <i className="bi bi-pencil me-2"></i>Edit Address
            </h5>
            <button 
              type="button" 
              className="btn-close" 
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-check mb-3">
                <input 
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="editSetAsDefault">
          ...
          <div className="modal-footer">
            ...
            >
              <i className="bi bi-check-lg me-1"></i>Update Address
            </button>
          </div>
        </div>
      </div>
